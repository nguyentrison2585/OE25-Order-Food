$(document).ready(function() {
  if (typeof(Storage) !== "undefined") {
    if ($('#btn-reset').length) {
      var restaurantId = $('.fa-check-circle')[0].dataset.id;
      if (restaurantId.length > 0) {
        ...
        function checkToStartSession() {
          if (sessionStorage.getItem('restaurant-id') === null) {
            sessionStorage['restaurant-id'] = restaurantId;
          }
          else if (sessionStorage.getItem('restaurant-id') != restaurantId) {
            ...
            if (number == 1) {
              ...
              if (orderItems.length == 0) {
                ...
                for (let index = j;index < sessionStorage['storage-lenght'];index++) {
                 sessionStorageChange('order-name-', index);
                 sessionStorageChange('order-number-', index);
                 sessionStorageChange('order-price-', index);
        ...
        function sessionStorageChange(name, index) {
          sessionStorage[name + index] = sessionStorage[name + (index + 1)];
        }
      }
    }
  }
